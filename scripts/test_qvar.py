...
compare("10 km -> base",  lambda: str(QVar(quantity_type="distance", raw_value="10 km")._convert_to_si_units()),  lambda: str(unit.Quantity("10 km").to_base_units()))
compare("3 min -> base",  lambda: str(QVar(quantity_type="time",     raw_value="3 min")._convert_to_si_units()),  lambda: str(unit.Quantity("3 min").to_base_units()))
compare("20 m/s -> base", lambda: str(QVar(quantity_type="velocity", raw_value="20 m/s")._convert_to_si_units()), lambda: str(unit.Quantity("20 m/s").to_base_units()))

section("Edge cases")
compare("empty + empty", lambda: str(empty + empty), lambda: "nan")
compare("km + empty",    lambda: str(km_a + empty),  lambda: "EX: TypeError: Type mismatch: cannot operate on 'distance' and ''")
